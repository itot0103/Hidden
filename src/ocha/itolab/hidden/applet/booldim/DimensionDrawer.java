...
//	double aspect = 1.0, im = -1.0;
	...
	static DoubleBuffer model = DoubleBuffer.allocate(16);
	static DoubleBuffer proj = DoubleBuffer.allocate(16);
	static DoubleBuffer objPos = DoubleBuffer.allocate(3);
...
//		trans.setDrawerUtility(du);
	...
	public void setPosId(int x, int y) {
		dsp.setXdimensionId(x);
		dsp.setYdimensionId(y);
	}
	...
	public void drag(int xStart, int xNow, int yStart, int yNow, ArrayList selectlist) {
		dsp.select(xStart, xNow, yStart, yNow, selectlist, view, model, proj);
	}
