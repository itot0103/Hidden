...
public class SPIndividualDrawer implements GLEventListener {
	...
	SPIndividualTransformer trans = null;
	IndividualSet ps;

	int imageSize[] = new int[2];
	double indEdgeratio = 0.0;
	...
	/**
	 */
	public SPIndividualDrawer(int width, int height, GLCanvas glc){
		glcanvas = glc;
		imageSize[0] = width;
		imageSize[1] = height;
	...
	public void setTransformer(SPIndividualTransformer view) {
		this.trans = view;
...
//		trans.setDrawerUtility(du);
	...
	public void setIndividualEdgeRatio(double r) {
		indEdgeratio = r;
	}
	...
	void drawIndividualPlots() {
		...
		for(int i = 0; i < ps.getNumIndividual(); i++) {
			OneIndividual p1 = ps.getOneIndividual(i);
			for(int j = (i + 1); j < ps.getNumIndividual(); j++) {
				OneIndividual p2 = ps.getOneIndividual(j);
				double dissim = p1.getDissimilarity(j);
				if(dissim < indEdgeratio) {
					gl2.glBegin(gl2.GL_LINES);
					gl2.glVertex3d(p1.getX(), (p1.getY() + SHIFT_Y), -0.01);
					gl2.glVertex3d(p2.getX(), (p2.getY() + SHIFT_Y), -0.01);
					gl2.glEnd();
				}
			}
		}
